export default class Algorithms {
    ...
    static shortestPath(graph: Graph, start: number, end: number): ShortestPathResult {
        const distances: number[] = new Array(graph.getNumberOfNodes()).fill(Infinity);
        const previous: (number | null)[] = new Array(graph.getNumberOfNodes()).fill(null);
        distances[start] = 0;
    ...
    static isBipartite(graph: Graph): BipartiteResult {
        const color: (number | null)[] = new Array(graph.getNumberOfNodes()).fill(null);
